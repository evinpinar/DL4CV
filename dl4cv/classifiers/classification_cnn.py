...
class ClassificationCNN(nn.Module):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, kernel_size=7,
        ...
        P_h = ((H-1)*stride_conv + K - H)//2
        P_w = ((W-1)*stride_conv + K - W)//2

        h_pool = (H - pool)//stride_pool+1
        w_pool = (W - pool)//stride_pool+1

        self.conv = nn.Conv2d(C, F, K, stride=stride_conv, padding=(P_h, P_w)) #conv layer 1
        self.conv.weight.data.mul_(weight_scale)
        self.maxPool = nn.MaxPool2d(pool, stride=stride_pool) # Pooling layer
        self.fc1 = nn.Linear(F*h_pool*w_pool, hidden_dim) # Affine layer 1
        self.drop = nn.Dropout(p=dropout) # Dropout layer
        self.fc2 = nn.Linear(hidden_dim, num_classes) # Affine layer 2 
