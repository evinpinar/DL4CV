      ...
      for f in range(F):
        for h_ in range(h_out):
          for w_ in range(w_out):
              ...
              w1 = w_*s
              w2 = w_*s+WW
              wind = x_pad[n, :, h1:h2, w1:w2]
              wind2 = w[f,:]
              conv = np.sum(np.multiply(wind, wind2))
              out[n, f, h_, w_] = conv + b[f]
      ...
      for f in range(F):
        for h_ in range(hh):
          for w_ in range(ww):
            ...
            w2 = w_*s+WW
            wind = im[:, h1:h2, w1:w2]
            dw[f, :] += wind*dout[n, f, h_, w_]
            db[f] += dout[n, f, h_, w_]
            dx[n, :, h1:h2, w1:w2] += w[f,:]*dout[n, f, h_, w_]
      ...
      for f in range(C):
        for h in range(hh):
          for w in range(ww):
            ...
            w1 = w*s
            w2 = w*s + ph
            im = x[i, f, h1:h2, w1:w2]
            out[i, f, h, w] = np.amax(im)
            #maxIdx[i, f, h, w, 0] = np.argmax(np.max(im, axis = 0))
            #maxIdx[i, f, h, w, 1] = np.argmax(np.max(im, axis = 1))
            #maxIdx[i, f, h, w, 0] = np.argmax(im)
      ...
      for f in range(C):
        for h in range(hh):
          for w in range(ww):
            ...
            w1 = w*s
            w2 = w*s + ph
            im = x[n, f, h1:h2, w1:w2]
            maxx = np.amax(im)
            mask = im == maxx
            dx[n, f, h1:h2, w1:w2] += dout[n,f,h,w] * mask
